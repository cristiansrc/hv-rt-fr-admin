...
describe("EducationPage coverage", () => {
  ...
  it("handles onPressEnter for highlights (lines 279-311)", async () => {
    const user = userEvent.setup();
    let currentValue: string[] = [];
    const setFieldsValueMock = vi.fn((values: any) => {
      if (values?.highlights) {
        currentValue = values.highlights;
      }
    });
    const getFieldValueMock = vi.fn((name: string) => {
      if (name === "highlights") return currentValue;
      return [];
    });
    ...
    // Add highlight using Enter key
    const highlightInputs = screen.getAllByPlaceholderText(/agregar logro/i);
    const highlightInput = highlightInputs[0];
    await user.type(highlightInput, "Highlight with Enter");
    await user.keyboard("{Enter}");

    await new Promise(resolve => setTimeout(resolve, 50));
    
    // Verify setFieldsValue was called
    expect(setFieldsValueMock.mock.calls.length).toBeGreaterThan(0);
    
    // Check if any call has the highlight
    const hasHighlight = setFieldsValueMock.mock.calls.some(
      call => call[0]?.highlights?.includes("Highlight with Enter")
    );
