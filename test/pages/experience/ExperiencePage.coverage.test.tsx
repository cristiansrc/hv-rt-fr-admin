...
describe("ExperiencePage coverage", () => {
  ...
  it("handles onPressEnter for descriptionItemsPdf (lines 327-328)", async () => {
    const user = userEvent.setup();
    let currentValue: string[] = [];
    const setFieldsValueMock = vi.fn((values: any) => {
      if (values?.descriptionItemsPdf) {
        currentValue = values.descriptionItemsPdf;
      }
    });
    const getFieldValueMock = vi.fn((name: string) => {
      if (name === "descriptionItemsPdf") return currentValue;
      return [];
    });
    ...
    const hasDesc = setFieldsValueMock.mock.calls.some(
      call => call[0]?.descriptionItemsPdf?.includes("Desc with Enter")
    );
    expect(hasDesc).toBe(true);
  });

  it("handles onPressEnter for descriptionItemsPdfEng (lines 362-363)", async () => {
    const user = userEvent.setup();
    let currentValue: string[] = [];
    const setFieldsValueMock = vi.fn((values: any) => {
      if (values?.descriptionItemsPdfEng) {
    ...
    // Add English description using Enter key
    const pdfEngInputs = screen.getAllByPlaceholderText(/agregar descripción pdf en inglés/i);
    const pdfEngInput = pdfEngInputs[0];
    await user.type(pdfEngInput, "Desc Eng with Enter");
    await user.keyboard("{Enter}");
    ...
    expect(setFieldsValueMock.mock.calls.length).toBeGreaterThan(0);
    const hasDesc = setFieldsValueMock.mock.calls.some(
      call => call[0]?.descriptionItemsPdfEng?.includes("Desc Eng with Enter")
    );
    expect(hasDesc).toBe(true);
  });
